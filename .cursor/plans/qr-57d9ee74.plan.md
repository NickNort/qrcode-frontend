<!-- 57d9ee74-8ba0-44a3-b897-c24de841df0b 7c7ec01d-e18b-4255-9e1e-054cc6b0800e -->
# QR Maker Page with shadcn Card and Form

### What we'll do

- **Install shadcn UI components** needed for the page: `button`, `card`, `input`, `label`, `select`.
- **Replace** `src/app/page.tsx` with a client component rendering a centered **Card** titled "Nice QR Maker" and a form collecting the required fields.
- **Add a placeholder submit** handler (logs values and includes a TODO to send to API).

### Commands to run

Run once from the project root to scaffold the UI primitives:

```bash
pnpx shadcn@canary add --yes button card input label select
```

This will generate components under `src/components/ui/` per your `components.json` aliases.

### File to change

- `src/app/page.tsx`: replace with the implementation below.

### Implementation sketch (essential parts)
...
- No API integration yet—only a console log and TODO comment.
- Uses Tailwind v4 utilities present in your `globals.css`.

### To-dos

- [ ] Add shadcn UI components: button, card, input, label, select
- [ ] Replace src/app/page.tsx with Card-based form and placeholder submit